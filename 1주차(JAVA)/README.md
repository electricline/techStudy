...
## Array(배열)의 장단점에 대해 설명해 주세요
- 장점 : 엑세스가 빠르다. 
- 단점 : 늘거나 줄지 못한다. 메모리를 차지하고 있다. (즉 가변성이 없다)

## List(리스트)의 장단점에 대해 설명해 주세요
- 장점 : List는 갯수를 안만들어도 되서 좋다.
       Array와 반대로 늘거나 줄어든다. 가
	   가변적이기 때문에, 메모리를 효율적으로 사용할수 있다.
- 단점 : 엑세스가 느리다.

## Map(맵)의 장단점에 대해 설명해 주세요
- 장점 : 엑세스가 빠르면서(배열), 늘어나거나 줄어들수 있다(리스트)
- 단점 : 메모리를 많이 쓴다. o(1)

## 동적(가변) 자료구조는 무엇이 있을까요?
- 가변은 List, Map, Set이 있고, 정적 자료구조는 Array로 구분됩니다.
- Array와 ArrayList랑 아애 다르다 ArrayList는 배열이 아니다. 

## LinkedList에 대해 설명해 주세요 
- 떨어져있는 리스트를 연결노드해서 쉽게 찾을수 있습니다. 
- 링크드 리스트를 양쪽 방향 연결한것이 double LinkedList 입니다.

## size가 1000인 배열을 만들습니다. 그 배열에 20개를 넣거나 5개로 줄이려면 어떻게 해야 하나요?
- 배열은 새로운 배열을 만들어서 복사 해놓고 기존 배열은 지워야 합니다.
- 배열 자체가 늘리거나 줄일순 없기 때문에, 갯수가 가변적이면 ArrayList를 사용하는것이 좋을 것 같습니다.
...
## 자바로 동적배열을 생성하고싶다면...
- 자바의 배열은 동적 할당되지 않으므로 동적할당이 필요할 때 연결리스트 사용합니다.
...
## stack, heap 영역별로 무엇이 저장되는지 설명해 주세요.
...
-> (참고 영상: [10분 테코톡] 🎅무민의 JVM Stack & Heap) 영역별로 설명
...
## Queue를 이용해 선입선출하는 메시지발송을 만들고 있는데, 만약에 긴급발송건이나 예약발송건이 있으면 먼저 보내줘야 합니다.
어떤 자료구조를 이용하는 것이 좋을까요? -> PriorityQueue(우선순위큐) Comparator 방식으로 구현 합니다.
...
## 7. 오버로딩과 오버라이딩의 차이점
   - 오버로딩은 메서드 이름들이 같아도 매개변수가 다르게 만들 수있습니다.
   - 오버라이딩은 상속받은 클래스나 인터페이스의 메소드를 재정의 할수있습니다.

   * 오버라이딩시 발생하는 동적바인딩에 대해 설명해주세요
   -   동적바인딩이란 다형성을 사용하여 메소드를 호출할 때 발생하는 현상이다. 런타임에 성격이 결정되며 실제 참조하는 객체는 서브 클래스이니 서브 클래스의 메소드를 호출하게 된다.

   * 오버로딩의 정적 바인딩에 대해 설명해주세요.
   -   정적바인딩이란 컴파일 시간에 성격이 결정되는 것을 말한다.

## 8. Access Modifier (접근 제한자)
...
## 10. Multi-Thread 환경에서의 개발
   ...
   * 동기화 synchronized {}에 붙을때와 메소드에 붙을때 차이 (동기화 메소드와 동기화 블록 차이점??)
   ...
   * 멀티쓰레드에서 synchronized를 안쓰고 synchronized를 하는방법
